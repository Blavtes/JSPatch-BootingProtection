...
static NSString *const kStartupCrashForTest = @"Boot_StartupCrashForTest"; // 尝试制造启动 crash
static NSString *const kContinuousCrashOnLaunchCounterKey = @"Boot_ContinuousCrashOnLaunchCounter";
static NSString *const kContinuousCrashFixingKey = @"Boot_ContinuousCrashFixing"; // 是否正在修复
static NSInteger const kContinuousCrashOnLaunchNeedToReport = 5; //crash 上报次数
static NSInteger const kContinuousCrashOnLaunchNeedToFix = 5; //crash 次数
...
+ (BOOL)launchContinuousCrashProtect
{
    ...
    if (launchCrashes >= kContinuousCrashOnLaunchNeedToFix) {
        if (Logger) Logger(@"app need to repair");
        ...
        if ([fileMgr fileExistsAtPath:filePath]) {
            ...
            for (NSString *subFileName in subFileArray) {
                ...
                if ([fileMgr removeItemAtPath:subFilePath error:nil]) {
                    if (Logger) Logger([NSString stringWithFormat:@"removed file path:%@", subFilePath]);
                } else {
                    if (Logger) Logger([NSString stringWithFormat:@"failed to remove file path:%@", subFilePath]);
                }
            }
        } else {
            if (Logger) Logger([NSString stringWithFormat:@"failed to remove non-existing file path:%@", filePath ]);
        }
    }
    
    if (Logger) Logger([NSString stringWithFormat:@"recoverFromContinuousCrash finished, files at home:[%@]\nDocuments:[%@]\nLibrary:[%@]\nCaches:[%@]",
